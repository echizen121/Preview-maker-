...
from backend.auto_alpha import AutoAlphaError, AutoAlphaJobManager, process_video_asset
...
alpha_jobs = AutoAlphaJobManager(ROOT)
...
@app.post("/api/auto-alpha")
async def api_auto_alpha(file: UploadFile, mode: str = Form("auto_alpha")) -> dict[str, Any]:
    try:
        return process_video_asset(ROOT, file.filename or "motion.mkv", await file.read(), mode)
    except AutoAlphaError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@app.post("/api/auto-alpha/start")
async def api_auto_alpha_start(file: UploadFile, mode: str = Form("auto_alpha")) -> dict[str, str]:
    job_id = alpha_jobs.start(file.filename or "motion.mkv", await file.read(), mode)
    return {"job_id": job_id}


@app.get("/api/auto-alpha/status/{job_id}")
def api_auto_alpha_status(job_id: str) -> dict[str, Any]:
    status = alpha_jobs.status(job_id)
    if not status:
        raise HTTPException(status_code=404, detail="透過ジョブが見つかりません。")
    return status
