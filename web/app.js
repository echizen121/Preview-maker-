...
let latestAlphaAsset = "";
let alphaStatusTimer = null;
let droppedAlphaFile = null;
let currentAlphaJobId = "";
...
function alphaFile() {
  return $("alphaInput").files[0] || droppedAlphaFile || null;
}

function selectedAlphaMode() {
  return $("alphaMode").value || "auto_alpha";
}

function setAlphaStatus(message) {
  $("alphaStatus").textContent = message;
}

function setAlphaResult(state, message, path = "", color = "") {
  const result = $("alphaResult");
  result.classList.toggle("done", state === "done");
  result.classList.toggle("error", state === "error");
  $("alphaResultMessage").textContent = message;
  $("alphaResultPath").textContent = path ? `登録先: ${path}` : "";
  $("alphaResultColor").textContent = color ? `推定背景色: ${color}` : "";
}

function startAlphaStatusTimer() {
  stopAlphaStatusTimer();
  const steps = ["アップロード中", "解析中", "外縁色を集計中", "背景色を推定中", "透過処理中", "素材として登録中"];
  let index = 0;
  $("alphaStatus").classList.add("processing");
  setAlphaStatus(steps[index]);
  alphaStatusTimer = window.setInterval(() => {
    index = Math.min(index + 1, steps.length - 1);
    setAlphaStatus(steps[index]);
  }, 1400);
}

function stopAlphaStatusTimer() {
  if (alphaStatusTimer) window.clearInterval(alphaStatusTimer);
  alphaStatusTimer = null;
  $("alphaStatus").classList.remove("processing");
}
...
function updateAlphaFileName() {
  const file = alphaFile();
  $("alphaFileName").textContent = file ? file.name : "MKV / MP4 / MOV / WebM";
  latestAlphaAsset = "";
  $("alphaPreview").removeAttribute("src");
  $("alphaPreview").load();
  setAlphaResult("", "未処理");
  if (file) {
    setAlphaStatus("動画を受け取りました。自動透過を開始します。");
  } else {
    setAlphaStatus("動画を選択してください。");
  }
}

async function processAlphaAsset() {
  const file = alphaFile();
  if (!file) throw new Error("動画ファイルを選択してください。");
  const mode = selectedAlphaMode();
  stopAlphaPolling();
  setAlphaStatus("アップロード中");
  $("alphaStatus").classList.add("processing");
  setAlphaResult("", mode === "auto_alpha" ? "自動透過ジョブを開始しています" : "通常素材登録ジョブを開始しています");
  latestAlphaAsset = "";
  await waitForPaint();
  const body = new FormData();
  body.append("file", file);
  body.append("mode", mode);
  const data = await api("/api/auto-alpha/start", { method: "POST", body });
  currentAlphaJobId = data.job_id;
  setAlphaStatus(`ジョブ開始: ${currentAlphaJobId}`);
  setAlphaResult("", "バックエンドで処理中です");
  await pollAlphaJob();
  alphaPollTimer = window.setInterval(() => {
    pollAlphaJob().catch((error) => {
      finishAlphaJob("error", error.message);
    });
  }, 1000);
}

async function pollAlphaJob() {
  if (!currentAlphaJobId) return;
  const data = await api(`/api/auto-alpha/status/${currentAlphaJobId}`);
  const lines = data.logs || [];
  const latestLine = lines.length ? lines[lines.length - 1] : data.status;
  setAlphaStatus(latestLine);
  log(lines);
  if (data.status === "done") {
    await applyAlphaResult(data.result || {});
    finishAlphaJob("done");
  } else if (data.status === "error") {
    finishAlphaJob("error", data.error || "透過処理に失敗しました。");
  }
}

async function applyAlphaResult(result) {
  if (!result.path) {
    throw new Error("透過ジョブは完了しましたが、生成素材のパスが返りませんでした。");
  }
  latestAlphaAsset = result.path;
  $("alphaPreview").src = `${assetUrl(result.path)}?v=${Date.now()}`;
  $("alphaPreview").dataset.path = result.path;
  $("alphaPreview").load();
  await refreshAssets();
  fields.motion.value = result.path;
  clearExternalAsset("motion");
  motionProxySource = "";
  motionProxyBuilding = "";
  updatePreview();
  setAlphaStatus(`素材化完了: ${result.path}`);
  setAlphaResult(
    "done",
    result.mode === "as_is" ? "通常動画素材を編集画面に反映しました" : "透過済み素材を編集画面に反映しました",
    result.path,
    result.estimated_color || ""
  );
}

function stopAlphaPolling() {
  if (alphaPollTimer) window.clearInterval(alphaPollTimer);
  alphaPollTimer = null;
  currentAlphaJobId = "";
}

function finishAlphaJob(status, message = "") {
  stopAlphaPolling();
  $("alphaStatus").classList.remove("processing");
  if (status === "error") {
    setAlphaStatus(`処理失敗: ${message}`);
    setAlphaResult("error", "透過処理に失敗しました", "", "");
    log(message);
  }
}
...
function bindEvents() {
  ...
  $("alphaMode").addEventListener("change", () => {
    setAlphaStatus("処理方式を変更しました。次に投入する動画へ適用します。");
  });
  $("alphaPreview").addEventListener("error", () => {
    if (latestAlphaAsset) {
      setAlphaStatus(`生成素材は登録済みですが、ブラウザで直接再生できません: ${latestAlphaAsset}`);
    }
  });
  $("alphaPreview").addEventListener("loadeddata", () => {
    if (latestAlphaAsset) {
      setAlphaStatus(`確認画面に反映しました: ${latestAlphaAsset}`);
    }
  });
  ["dragenter", "dragover"].forEach((eventName) => {
    $("alphaDropZone").addEventListener(eventName, (event) => {
      event.preventDefault();
      $("alphaDropZone").classList.add("drag-over");
    });
  });
  ["dragleave", "drop"].forEach((eventName) => {
    $("alphaDropZone").addEventListener(eventName, (event) => {
      event.preventDefault();
      $("alphaDropZone").classList.remove("drag-over");
    });
  });
  $("alphaDropZone").addEventListener("drop", (event) => {
    const file = event.dataTransfer.files[0];
    if (!file) return;
    droppedAlphaFile = file;
    $("alphaInput").value = "";
    updateAlphaFileName();
    processAlphaAsset()
      .catch((error) => {
