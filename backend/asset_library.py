...
DIRECTORIES = [
    ...
    "projects/product_001/alpha_cache",
    "resources",
]
...
def collect_motion_files(root: Path) -> list[dict[str, str]]:
    return [
        ...
        if is_visible_motion_asset(Path(item["path"]))
    ]
...
def collect_preset_files(root: Path) -> list[dict[str, str]]:
    return [
        item
        for item in collect_files(root, ["templates/presets", "projects"], PRESET_EXTS)
        if not any(part in {"output", "preview_cache", "alpha_cache"} for part in Path(item["path"]).parts)
    ]


def is_visible_motion_asset(path: Path) -> bool:
    parts = path.parts
    if any(part in {"output", "preview_cache"} for part in parts):
        return False
    if "alpha_cache" in parts and "source" in parts:
        return False
    return True
...
def asset_library(root: Path) -> dict[str, list[dict[str, str]]]:
    ...
    return {
        ...
        "presets": collect_preset_files(root),
